...
// Funkcja wysłania emaila
function sendPasswordResetEmail($email, $username, $resetLink) {
    $mail = new PHPMailer(true);
    
    try {
        $mail->isSMTP();
        $mail->Host = 'hosting2626806.online.pro';
        $mail->SMTPAuth = true;
        $mail->Username = 'admin@446dx.pl';
        $mail->Password = 'Joanna2008@!!';
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;
        
        $mail->setFrom('admin@446dx.pl', '446DX.PL');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        if (!$user) {
            ...
            // Wyślij email
            $resetLink = "http://localhost/446cluster/reset-password.php?token=$resetToken";
